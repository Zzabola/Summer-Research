...
//                    if (sb_req) begin
//                        if (sb_we) begin
//                            // Start m_axi_breadyWrite Transaction
...
//    if (!kicked) begin
//      boot_cnt <= boot_cnt + 1;
//      if (boot_cnt == 32'd1000) begin  // ~0.1s at 50MHz
...
//        end
  ...
  if (!rst_n) begin
    ...
    // default: ready only when idle
    sb_ready <= (state == IDLE);

    case (state)
      IDLE: begin
        // hard deassert to prevent "stuck valids"
        m_axi_awvalid <= 1'b0;
        m_axi_wvalid  <= 1'b0;
        m_axi_bready  <= 1'b0;
        m_axi_arvalid <= 1'b0;
        ...
        if (sb_req && sb_ready) begin
          if (sb_we) begin
            ...
            // don't assert bready yet (optional but recommended)
            m_axi_bready  <= 1'b0;
      ...
      WRITE: begin
        ...
        // only start accepting a response after both handshakes happened
        if (aw_done && w_done)
          m_axi_bready <= 1'b1;

        // complete only when it's a real response for our issued write
        if (aw_done && w_done && m_axi_bvalid && m_axi_bready) begin
          sb_rvalid    <= 1'b1;
          sb_rdata     <= 32'b0;
          m_axi_bready <= 1'b0;
          state        <= IDLE;
